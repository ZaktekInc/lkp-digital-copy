...
export default function LicensesPanel() {
  ...
  function save() {
    ...
    setNotice("");
    try {
      window.LkpBrowserStore.updateActivation(selected.id, { comment: selected.comment });
  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.7fr)]">
      ...
      <aside className="rounded-xl border border-[#dce3ec] bg-white p-5 shadow-sm">
        {!selected ? <p className="text-[#65758b]">Выберите активацию в таблице.</p> : <div className="space-y-4"><div><p className="text-sm text-[#65758b]">Активация</p><h3 className="text-xl font-bold">{selected.number}</h3><p>{selected.organization.name}</p></div><div className="rounded-lg bg-[#f5f7fa] p-3"><span className="block text-sm text-[#65758b]">Статус</span><strong className="block">{selected.status}</strong><span className="mt-3 block text-sm text-[#65758b]">Вендор</span><strong className="block">{selected.vendor}</strong><span className="mt-3 block text-sm text-[#65758b]">Оплата</span><strong className="block">{selected.paymentStatus}</strong></div><label className="block text-sm">Комментарий<textarea value={selected.comment} onChange={(event) => setSelected({ ...selected, comment: event.target.value })} rows={3} className="mt-1 w-full rounded-lg border border-[#b8c7da] px-3 py-2" /></label><button type="button" onClick={save} className="rounded-lg bg-[#1769c2] px-4 py-2 text-sm font-semibold text-white">Сохранить комментарий</button>{notice && <p className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900" role="status">{notice}</p>}<div><h4 className="mb-2 font-semibold">Позиции и ключи</h4><div className="space-y-3">{selected.items.map((item) => <div key={item.id} className="rounded-lg border border-[#e5eaf1] p-3 text-sm"><strong>{item.model}</strong><p className="text-[#65758b]">{item.licenseType}</p><p className="mt-2">Ключей: {item.licenseKeys.length}</p>{item.licenseKeys.map((key) => <p key={key.id} className="text-[#65758b]">{key.serialNumber} · {key.status}</p>)}</div>)}</div></div></div>}
      </aside>
    </section>
  );
}
