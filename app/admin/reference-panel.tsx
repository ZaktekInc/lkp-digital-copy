...
export default function ReferencePanel({ kind, title }: { kind: string; title: string }) {
  ...
  return (
    <section className="rounded-xl border border-[#dce3ec] bg-white p-5 shadow-sm">
      ...
      <div className={`mt-5 grid gap-5 ${creating || selected ? "lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]" : ""}`}>
        <div className="overflow-x-auto rounded-lg border border-[#e5eaf1]">
          {loading ? <p className="p-5 text-[#65758b]">Загрузка…</p> : items.length === 0 ? <p className="p-5 text-[#65758b]">Записей пока нет.</p> : (
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody>{items.map((item) => <tr key={item.id} onClick={() => { setSelected(item); setCreating(false); }} className={`cursor-pointer border-t border-[#edf0f4] hover:bg-[#f7faff] ${selected?.id === item.id ? "bg-[#edf5ff]" : ""}`}><td className="px-4 py-3 font-mono text-xs">{item.code}</td><td className="px-4 py-3 font-semibold">{item.name}</td><td className="px-4 py-3"><BooleanFlag value={item.isActive} label={item.isActive ? "Активна" : "Неактивна"} /></td></tr>)}</tbody>
            </table>
          )}
        </div>
        {(creating || selected) && <div className="space-y-5">
          {creating && <form key={`create-${kind}`} onSubmit={createItem} className="space-y-3 rounded-lg bg-[#f5f7fa] p-4">
            ...
            <div className="flex gap-2"><button disabled={saving} className="rounded-lg bg-[#1769c2] px-4 py-2 font-semibold text-white disabled:opacity-50">Добавить</button><button type="button" onClick={() => setCreating(false)} className="rounded-lg border border-[#b8c7da] px-4 py-2">Отмена</button></div>
          </form>}
